...
show_initial_state() {
    ...
    kubectl get applications -n argocd | grep -E '(dev-api-app|dev-demo-app|NAME)' || echo "No matching applications found"
    echo

    log_step "Current Pod Count in Each Namespace:"
    echo "Dev API App namespace:"
    kubectl get pods -n dev-api-app --no-headers 2>/dev/null | grep -v 'Completed' | wc -l || echo "0"
    echo "Dev Demo App namespace:"
    kubectl get pods -n dev-demo-app --no-headers 2>/dev/null | grep -v 'Completed' | wc -l || echo "0"
    echo
    ...
    local current_rev
    current_rev=$(kubectl get application dev-api-app -n argocd -o jsonpath='{.status.sync.revision}' 2>/dev/null | cut -c1-8 || echo "Unknown")
...
monitor_selective_sync() {
    ...
    initial_api_status=$(kubectl get application dev-api-app -n argocd -o jsonpath='{.status.sync.status}' 2>/dev/null || echo "Unknown")
    initial_api_rev=$(kubectl get application dev-api-app -n argocd -o jsonpath='{.status.sync.revision}' 2>/dev/null | cut -c1-8 || echo "Unknown")
    initial_api_replicas=$(kubectl get deployment -n dev-api-app -o jsonpath='{.items[0].spec.replicas}' 2>/dev/null || echo "0")
    ...
    while [ $(($(date +%s) - start_time)) -lt 30 ]; do  # 30s max for detection
        ...
        api_rev=$(kubectl get application dev-api-app -n argocd -o jsonpath='{.status.sync.revision}' 2>/dev/null | cut -c1-8 || echo "Unknown")
...
show_results() {
    ...
    kubectl get applications -n argocd | grep -E '(dev-api-app|dev-demo-app|NAME)' || echo "No applications found"
    echo

    # Pod counts
    log_step "Pod Counts After Change:"
    local api_pods demo_pods
    api_pods=$(kubectl get pods -n dev-api-app --no-headers 2>/dev/null | grep -v 'Completed' | wc -l || echo "0")
    demo_pods=$(kubectl get pods -n dev-demo-app --no-headers 2>/dev/null | grep -v 'Completed' | wc -l || echo "0")
    ...
    local workflows
    workflows=$(kubectl get workflows --all-namespaces --no-headers 2>/dev/null | grep -E '(dev-api-validation|dev-demo-validation)' | wc -l || echo "0")

    echo "Recent validation workflows: $workflows"

    if [ "$workflows" -gt 0 ]; then
        log_step "Workflow Details:"
        kubectl get workflows --all-namespaces -o custom-columns=NAME:.metadata.name,NAMESPACE:.metadata.namespace,STATUS:.status.phase,AGE:.metadata.creationTimestamp --no-headers 2>/dev/null | grep -E '(dev-api-validation|dev-demo-validation)' | head -3 || echo "  No workflow details available"
    fi
    echo
    ...
    kubectl get events -n argocd --field-selector involvedObject.name=dev-api-app --sort-by='.lastTimestamp' 2>/dev/null | tail -3 || echo "  No recent events"
    echo
    echo "📱 dev-demo-app events:"
    kubectl get events -n argocd --field-selector involvedObject.name=dev-demo-app --sort-by='.lastTimestamp' 2>/dev/null | tail -3 || echo "  No recent events"
    echo
}
...
analyze_results() {
    ...
    api_chart_version=$(kubectl get application dev-api-app -n argocd -o jsonpath='{.status.sync.revision}' 2>/dev/null || echo "Unknown")
    demo_chart_version=$(kubectl get application dev-demo-app -n argocd -o jsonpath='{.status.sync.revision}' 2>/dev/null || echo "Unknown")
